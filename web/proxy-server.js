...
function proxyRequest(req, res, targetHost, targetPort, prefixToStrip) {
  ...
  const proxyReq = http.request(options, (proxyRes) => {
    ...
    // 删除上游 CORS 头，避免与代理设置的 * 冲突（allow-credentials + * 会导致浏览器拒绝）
...
async function handleRequest(req, res) {
  ...
  if (parsedUrl.pathname === '/api/audio-proxy') {
    ...
    let referer = '';
    if (targetUrl.includes('music.126.net') || targetUrl.includes('music.163.com')) {
      referer = 'https://music.163.com/';
    } else if (targetUrl.includes('kuwo.cn')) referer = 'http://www.kuwo.cn/';
